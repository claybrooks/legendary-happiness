...
var unbounded = new UnboundedCommitHistory();
var bounded = new FixedSizeRollingCommitHistory((uint)size);
...
{
    unbounded.Commit(action);
}
timer.Stop();
Console.WriteLine($"Unbounded commit of {size}: {timer.Elapsed.TotalMilliseconds}");
...
timer.Stop();
Console.WriteLine($"Unbounded undo of {size}: {timer.Elapsed.TotalMilliseconds}");
...
timer.Stop();
Console.WriteLine($"Unbounded redo of {size}: {timer.Elapsed.TotalMilliseconds}");

timer.Reset();
timer.Start();
for (uint i = 0; i < size; ++i)
{
    bounded.Commit(action);
}
timer.Stop();
Console.WriteLine($"Bounded commit of {size}: {timer.Elapsed.TotalMilliseconds}");
...
timer.Stop();
Console.WriteLine($"Bounded undo of {size}: {timer.Elapsed.TotalMilliseconds}");
...
timer.Stop();
Console.WriteLine($"Bounded redo of {size}: {timer.Elapsed.TotalMilliseconds}");
